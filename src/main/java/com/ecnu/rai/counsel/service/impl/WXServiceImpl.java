...
public class WXServiceImpl implements WXService {
    ...
    @Override
    public List<VisitorSMInfo> getAllVisitor() {
        List<Visitor> visitors = visitorMapper.getVisitorList();
        List<VisitorSMInfo> visitorSMInfos = new ArrayList<>();
        for(Visitor visitor:visitors)
        {
            VisitorSMInfo visitorSMInfo = new VisitorSMInfo(visitor);
            visitorSMInfo.setState(userMapper.findStateById(visitor.getId()));
            visitorSMInfos.add(visitorSMInfo);
        }
        return visitorSMInfos;
    }
